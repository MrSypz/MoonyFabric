...
public class HellforkItem extends MamyTridentItem {
    public HellforkItem(Settings settings) {
        super(settings);
    }
    @Override
    ...
    protected boolean canRiptide(PlayerEntity playerEntity) {
        return playerEntity.isInLava() || playerEntity.isOnFire() || playerEntity.getMainHandStack().getItem() == ModItems.SOULFORK || playerEntity.getOffHandStack().getItem() == ModItems.SOULFORK;
    }
    ...
    public void onStoppedUsing(ItemStack stack, World world, LivingEntity user, int remainingUseTicks) {
        ...
        if (user instanceof PlayerEntity player && user.isUsingRiptide() && stack.getItem() == ModItems.SOULFORK) {
            if (player.experienceLevel <= 0) {
                user.damage(ModDamageTypes.create(world, ModDamageTypes.HELLFORK_HEAT_TYPE), 2f);
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        ...
        } else if (EnchantmentHelper.getRiptide(itemStack) > 0 && !user.isInLava() && !user.isOnFire() && !(itemStack.hasNbt() && itemStack.getItem() == ModItems.SOULFORK)) {
            return TypedActionResult.fail(itemStack);
        } else {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        if ((context.getWorld().getBlockState(context.getBlockPos()).getBlock() == Blocks.SOUL_CAMPFIRE || context.getWorld().getBlockState(context.getBlockPos()).getBlock() == Blocks.SOUL_LANTERN || context.getWorld().getBlockState(context.getBlockPos()).getBlock() == Blocks.SOUL_TORCH || context.getWorld().getBlockState(context.getBlockPos()).getBlock() == Blocks.SOUL_WALL_TORCH) && context.getStack().getItem() == ModItems.HELLFORK) {
            ItemStack soulfork = new ItemStack(ModItems.SOULFORK, context.getStack().getCount());

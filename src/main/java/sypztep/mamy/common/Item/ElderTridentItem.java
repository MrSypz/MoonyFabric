...
public class ElderTridentItem extends MamyTridentItem {
    public ElderTridentItem(Settings settings) {
        super(settings);
    }
    @Override
        ...
        if (useTime >= 10 && j > 0) {
            for (int i = 1; i <= j; i++) {
                if (!world.isClient && user instanceof PlayerEntity player) {
                    MamyTridentEntity trident = ModEntityTypes.GUARDIAN_TRIDENT.create(world);
                    if (trident == null) {
                        continue;
                    }
